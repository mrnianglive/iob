...
class BielletageManagerPDO extends BielletageManager
{
    ...
    {
        $requeteGetInvoice = $this->dao->prepare("SELECT TbleBilletage.*, TbleOperations.*, TbleUsers.PrenomUsers, TbleCaisse.*, TbleAgency.*, TbleProduit.*, TbleBanque.NameBanque FROM TbleBilletage INNER JOIN TbleOperations ON TbleOperations.RefOperations=TbleBilletage.RefOperations INNER JOIN TbleUsers ON TbleUsers.RefUsers=TbleOperations.Insert_Id INNER JOIN TbleCaisse ON TbleCaisse.RefCaisse=TbleOperations.RefCaisse INNER JOIN TbleAgency ON TbleAgency.RefAgency=TbleCaisse.RefAgency LEFT JOIN TbleProduit ON TbleProduit.RefProduit=TbleOperations.RefProduit LEFT JOIN TbleBanque ON TbleBanque.RefBanque=TbleProduit.RefBanque  WHERE  TbleBilletage.RefOperations=:RefOperations");
