    ...
    public function UserCaisse($Date, $Pays = NULL, $Agence = NULL, $Caisse = NULL)
{
    ...
    // Ajoutez cette méthode à la classe JournalManagerPDO

public function getAgencyDetails($agencyId, $date)
{
    $details = [];
    
    // Récupérer les informations de base de l'agence
    $agencyQuery = $this->dao->prepare('SELECT * FROM TbleAgency WHERE RefAgency = :agencyId');
    $agencyQuery->bindValue(':agencyId', $agencyId, \PDO::PARAM_INT);
    $agencyQuery->execute();
    $agencyInfo = $agencyQuery->fetch();
    
    $details['agencyName'] = $agencyInfo['NameAgency'];
    
    // Récupérer les détails des caisses de l'agence
    $caisseQuery = $this->dao->prepare('SELECT * FROM TbleCaisse WHERE RefAgency = :agencyId');
    $caisseQuery->bindValue(':agencyId', $agencyId, \PDO::PARAM_INT);
    $caisseQuery->execute();
    $caisses = $caisseQuery->fetchAll();
    
    $details['caisses'] = [];
    foreach ($caisses as $caisse) {
        $caisseDetails = [
            'NameCaisse' => $caisse['NameCaisse'],
            'SoldeInitial' => $this->SoldeInitialCaisse($date, $caisse['RefCaisse']),
            'TotalAppro' => $this->TotalApproCaisse($date, $caisse['RefCaisse']),
            'TotalSortieCaisse' => $this->TotalSortieCaisse($date, $caisse['RefCaisse']),
            'TotalVersement' => $this->SomnmeVersementCaisse($date, $caisse['RefCaisse']),
            'TotalRetrait' => $this->SommeRetraitCaisse($date, $caisse['RefCaisse']),
            'SoldeDisponible' => $this->SoldeActuelleCaisse($date, $caisse['RefCaisse'])
        ];
        
        if ($_SESSION['RefPays'] != 1) {
            $caisseDetails['TotalFraisTimbre'] = $this->TotalFraisTimbreCaisse($date, $caisse['RefCaisse']);
        }
        
        $details['caisses'][] = $caisseDetails;
    }
    
    // Récupérer les détails des dépôts et retraits par produit
    $details['depotProduit'] = $this->getDepotParProduit($agencyId, $date);
    $details['retraitProduit'] = $this->getRetraitParProduit($agencyId, $date);
    
    return $details;
}

// Ajoutez ces méthodes auxiliaires si elles n'existent pas déjà

private function getDepotParProduit($agencyId, $date)
{
    $query = $this->dao->prepare('SELECT p.NameProduit, SUM(o.MontantVersement) as total
                                  FROM TbleOperations o
                                  JOIN TbleProduit p ON o.RefProduit = p.RefProduit
                                  JOIN TbleCaisse c ON o.RefCaisse = c.RefCaisse
                                  WHERE c.RefAgency = :agencyId
                                  AND DATE(o.Approve2_Time) = :date
                                  AND o.RefType = 1
                                  GROUP BY p.RefProduit');
    $query->bindValue(':agencyId', $agencyId, \PDO::PARAM_INT);
    $query->bindValue(':date', $date, \PDO::PARAM_STR);
    $query->execute();
    return $query->fetchAll(\PDO::FETCH_KEY_PAIR);
}

private function getRetraitParProduit($agencyId, $date)
{
    $query = $this->dao->prepare('SELECT p.NameProduit, SUM(o.MontantVersement) as total
                                  FROM TbleOperations o
                                  JOIN TbleProduit p ON o.RefProduit = p.RefProduit
