                    ...
                    <tbody>
                        <?php foreach ($ListeUsers as $key => $users) { ?>
                            <tr>
                                ...
                                <td><?= $users['nomPays']; ?></td>
                                <td>
                                    <a href="/Users/doubleauth/<?= $users['RefUsers']; ?>" class="btn btn-info" data-toggle="tooltip" title="Activer l'authentification à deux facteurs"><i class="fas fa-lock"></i>2FA</a>
                                    <a href="/Users/UpdateUsers/<?= $users['RefUsers']; ?>" class="btn btn-info" data-toggle="tooltip" title="Modifier les informations de l'utilisateur"><i class="ti-pencil-alt"></i></a>
                                    <?php if ($_SESSION['statut'] == 'superadmin') { ?>
                                        <a href="/Users/deleteUsers/<?= $users['RefUsers']; ?>">
                                            <button type="button" class="btn btn-danger" data-toggle="tooltip" title="Cliquer pour supprimer l'utilisateur"><i class="fa fa-trash"></i></button></a>
                                    <?php } ?>
                                </td>
                                <td>
                                    <button type="button" class="btn btn-info " data-toggle="modal" data-target="#Chmod-<?= $users['RefUsers']; ?>" data-whatever="@mdo" title="Accès aux caisses"><i class="ticon ti-lock"></i></button>

                                    <?php if ($users['Name'] == 'caissier' or $users['Name'] == 'ChefCaisse' or $users['Name'] == 'superadmin' or $users['Name'] == 'admin') { ?>
                                        <button type="button" class="btn btn-warning " data-toggle="modal" data-target="#ChmodAppro-<?= $users['RefUsers']; ?>" data-whatever="@mdo" title="Accès aux caisses pour l'appo et la sortie de fond | Only Caissier et ChefCaisse "><i class="ticon ti-lock"></i></button>
                                    <?php } ?>
                                </td>
                            </tr>
